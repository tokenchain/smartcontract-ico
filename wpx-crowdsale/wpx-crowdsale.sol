...
// - Ensure max supply is 90,000,000
// - Ensure that even if not totalSupply is sold, tokens would still be transferrable after (we will up to totalSupply by creating APX tokens)

// vesting: 365 days, 365 days / 4 vesting


contract APXToken is VestedToken {
  //FIELDS
  string public name = "AorkPress2"; //(important input)
  string public symbol = "APX2";    //(important input)
...
// Multiplier for the decimals
  ...
  //CONSTANTS
  //Time limits
  uint public constant STAGE_ONE_TIME_END = 1 minutes; // first day bonus (important input)
  uint public constant STAGE_TWO_TIME_END = 2 minutes; // first week bonus (important input)
  uint public constant STAGE_THREE_TIME_END = 50 minutes; //(important input)
  

  //Prices of APX
  uint public constant PRICE_STANDARD    = 2000*DECIMALS; // APX received per one ETH; Approximately $0.12 if ETH price $300. MAX_SUPPLY / (valuation / ethPrice)
  uint public constant PRICE_STAGE_ONE   = PRICE_STANDARD * 150/100;  // 1ETH = 100% Bonus 1.5X or 50% 1ETH = 3000 APX ~ $0.10(important input)
  uint public constant PRICE_STAGE_TWO   = PRICE_STANDARD * 125/100; // 1ETH = 25% ICO Bonus           1ETH = 2500 APX ~ $0.125(important input)
  uint public constant PRICE_STAGE_THREE = PRICE_STANDARD; 			// Standard Price no bonus         1ETH = 2000 APX ~ $0.15(important input)
  ...
  address public apxTeamAddress; // Address to which ALLOC_TEAM, ALLOC_BOUNTIES, ALLOC_WINGS is (ultimately) sent to.
  ...
  uint public APXSold; // Total APX created
  ...
  function isCrowdfundCompleted() internal returns (bool) {
    if (now > publicEndTime || APXSold+3000*DECIMALS >= ALLOC_CROWDSALE || etherRaised >= hardcapInEth*1000000000000000000) {return true; }
  ...
  function APXToken(
    ...
    publicEndTime = _publicStartTime + 60 days; // (important input)
    multisigAddress = _multisig;
    apxTeamAddress = _apxTeam;
    ...
    balances[apxTeamAddress] += ALLOC_BOUNTIES;
    balances[apxTeamAddress] += ALLOC_WINGS;
  ...
  //constant function returns the current APX price.
  ...
  // calculates wmount of APX we get, given the wei and the rates we've defined per 1 eth
  ...
  // Returns `amount` in scope as the number of APX tokens that it will purchase.
  ...
  //Results in creation of new APX Tokens if transaction would not exceed hard limit of APX Token.
  function()
    payable
    is_crowdfund_period
    is_not_halted
  {
    uint amount = processPurchase(getPriceRate(), SafeMath.sub(ALLOC_CROWDSALE, APXSold));
  ...
  function grantVested(address _apxTeamAddress, address _apxFundAddress)
    is_crowdfund_completed
    only_owner
    is_not_halted
  {
    ...
    grantVestedTokens(
      _apxTeamAddress, ALLOC_TEAM,
      uint64(now), uint64(now) + 91 days , uint64(now) + 365 days, 
      false, false
    );

    // Grant tokens that remain after crowdsale to the APX fund, vested for 2 years
    grantVestedTokens(
      _apxFundAddress, balances[ownerAddress],
